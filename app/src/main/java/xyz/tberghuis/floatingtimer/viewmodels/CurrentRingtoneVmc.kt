...
class CurrentRingtoneVmc(
  private val alarmRingtoneUriFlow: Flow<String?>,
  private val customSoundNameFlow: Flow<String?>,
  ...
  init {
    scope.launch {
      // Manual combine or just launch
      kotlinx.coroutines.flow.combine(alarmRingtoneUriFlow, customSoundNameFlow) { uri, customName ->
        if (customName != null) {
          currentRingtoneUri = null
          currentRingtoneTitle = "Custom: $customName"
        } else {
